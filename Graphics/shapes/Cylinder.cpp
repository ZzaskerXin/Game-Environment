...
void Cylinder::makeWedge(float curT, float nexT)
{
    ...
    for (int i = 0; i < numDivisions; i++)
    {
        ...
        glm::vec3 nTop(0.f, 1.f, 0.f);

        glm::vec2 uvT0 = uvForCap(t2);
        glm::vec2 uvT1 = uvForCap(t0);
        glm::vec2 uvT2 = uvForCap(t3);
        glm::vec2 uvT3 = uvForCap(t1);


        insertVec3(m_vertexData, t0); insertVec3(m_vertexData, nTop); insertVec2(m_vertexData, uvT1);
        insertVec3(m_vertexData, t2); insertVec3(m_vertexData, nTop); insertVec2(m_vertexData, uvT0);
        insertVec3(m_vertexData, t3); insertVec3(m_vertexData, nTop); insertVec2(m_vertexData, uvT2);
        appendVertexData(t2, t0, t3);

        insertVec3(m_vertexData, t3); insertVec3(m_vertexData, nTop); insertVec2(m_vertexData, uvT2);
        insertVec3(m_vertexData, t0); insertVec3(m_vertexData, nTop); insertVec2(m_vertexData, uvT1);
        insertVec3(m_vertexData, t1); insertVec3(m_vertexData, nTop); insertVec2(m_vertexData, uvT3);
        appendVertexData(t3, t0, t1);

        glm::vec3 b0(t0.x, -t0.y, t0.z);
        glm::vec3 b1(t1.x, -t1.y, t1.z);
        glm::vec3 b2(t2.x, -t2.y, t2.z);
        glm::vec3 b3(t3.x, -t3.y, t3.z);

        glm::vec3 nBot(0.f, -1.f, 0.f);
